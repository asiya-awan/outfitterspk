...
import com.outfitterspk.jdbc.WomenProductDBUtil;
import com.outfitterspk.models.Product;
import com.outfitterspk.models.ProductUtil;
import com.outfitterspk.models.ProductVariant;


@WebServlet("/WomenProductsServlet")
public class WomenProductsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	private WomenProductDBUtil womenProductDBUtil;
	...
	public void init() throws ServletException {
		...
		try {
			womenProductDBUtil=new WomenProductDBUtil(dataSource);
		}
		catch(Exception exc) {
...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    private void listProducts(HttpServletRequest request, HttpServletResponse response) 
		...
		List<Product> products= womenProductDBUtil.getWomenProducts();
		
		//add products to the request
		request.setAttribute("WOMEN_PRODUCTS_ALL", products);
		
		//send to JSP page(view)
		RequestDispatcher dispatcher= request.getRequestDispatcher("view-women.jsp");
    ...
    private void loadProduct(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		//get product from database (db util)
			Product theProduct = womenProductDBUtil.getWomenProduct(theProductId);
			
		//get product from database (db util)
			List<ProductVariant> theProductVariant = womenProductDBUtil.getWomenProductVariant(theProductId);
